(function () {
  ...
  var storageNamespace = (window.location.pathname || 'memory-palace-builder').toLowerCase().replace(/[^a-z0-9]+/g, '-');
  var STORAGE_KEY = 'memory-palace-builder-v1-' + storageNamespace;
  var UI_STORAGE_KEY = 'memory-palace-builder-ui-v1-' + storageNamespace;
  ...
  function loadPreset() {
    ...
    state.review.blockId = null; state.review.roomIndex = 0;
    sketchUndoStack = []; sketchRedoStack = [];
    render(true); setStatus('Loaded Chapter 7 preset.');
  }
  ...
  byId('reset-all-data-button').addEventListener('click', function () {
    showConfirm('Reset all data?', 'Erase every palace from localStorage and reload the Chapter 7 preset?', function () {
