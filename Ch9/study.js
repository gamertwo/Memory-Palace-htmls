(function () {
  ...
  function decorateBuilderHref(rawHref) {
    try {
      var href = String(rawHref || '');
      var url = new URL(href, window.location.href);
      if (!/memory-palace-builder\.html$/i.test(url.pathname)) return href;
      if (!url.searchParams.get('pages')) {
        url.searchParams.set('pages', getPageFilename());
      }
      url.searchParams.set('handoff', '1');
      return url.pathname.split('/').pop() + url.search + url.hash;
    } catch (error) {
      var join = String(rawHref || '').indexOf('?') >= 0 ? '&' : '?';
      return String(rawHref || '') + join + 'handoff=1';
    }
  }
  ...
  function addBuilderLinks() {
    ...
    heroToolRows.forEach(function (row) {
      ...
      link.href = decorateBuilderHref('memory-palace-builder.html?pages=' + encodeURIComponent(pageFilename));
    ...
    document.querySelectorAll('.room').forEach(function (room) {
      ...
      link.href = decorateBuilderHref('memory-palace-builder.html?pages=' + encodeURIComponent(pageFilename) + '&focus=' + encodeURIComponent(roomId));
  ...
  document.addEventListener('click', function (event) {
    var link = event.target.closest('a[href*="memory-palace-builder.html"]');
    if (!link) return;
    storeBuilderHandoff();
    link.href = decorateBuilderHref(link.getAttribute('href'));
  });
})();
